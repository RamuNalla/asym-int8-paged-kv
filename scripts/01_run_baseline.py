...
def main():
    ...
    attention = MinimalAttention().to(device)
    attention.eval()
    ...
    ttft_no, latencies_no = simulate_inference(attention, prefix_len, gen_steps, use_cache=False, device=device)
    
    print("Running Standard Cache baseline...")
    ttft_cache, latencies_cache = simulate_inference(attention, prefix_len, gen_steps, use_cache=True, device=device)
